...
from .utils import resize_and_crop, get_square, normalize, hwc_to_chw, resize_and_crop_anno
...
def to_cropped_anno(ids, dir, suffix, scale):
    """From a list of tuples, returns the correct cropped img"""
    for id, pos in ids:
        im = resize_and_crop_anno(Image.open(dir + id + suffix), scale=scale)
        yield get_square(im, pos)
